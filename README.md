...
`assets/genomes.json` at the repo root ("Reference files" above) — so moving or deleting the clone
breaks them. The scripts resolve their own real path through the symlink, so with the symlink install
above nothing else needs linking: `assets/genomes.json` is found inside the clone.
