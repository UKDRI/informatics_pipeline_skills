#!/bin/bash
#
#SBATCH --job-name=nf-core:spatialvi     # Job name
...
# dev build — commit d0fd35d; confirm path on cluster
main=/nfsdata/scripts/nf-core/dev/spatialvi/main.nf
...
# All non-default pipeline parameters (spaceranger_reference, filtering thresholds, ...) are set in params.yml
$exec run $main \
   -profile apptainer \
   --input $samplesheet \
   --outdir $outdir \
